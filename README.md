...
Enable VCS coverage when coverage results should appear in the dashboard:
...
Coverage metrics and columns are shown only for VCS regressions with
`--vcs-cm` enabled. VCS regressions without coverage and all XRUN reports omit
the coverage UI instead of filling it with `N/A` values.
Aggregate project tables show coverage columns only when every displayed bench
has VCS coverage; mixed VCS/XRUN projects keep those columns on the VCS bench
detail pages so XRUN rows never acquire coverage placeholders.
...
When a VCS coverage regression has failed tests, simmer preserves the merged
coverage from its passing tests and writes a per-bench `<timestamp>.rerun.json`
manifest. The failed report includes a **Copy failed rerun command** button.
Run the copied command on the configured VCS workstation:

```bash
simmer --rerun-report '/absolute/path/to/<timestamp>.rerun.json'
```

Each failed seed is rerun separately. Coverage from successful reruns is merged
with the preserved baseline and published as a new timestamped revision; the
original report remains unchanged within the existing 30-run retention window.
A partially successful rerun updates only
the successful tests and writes another manifest for the failures that remain.
If no failed test passes, simmer leaves the report and baseline unchanged.
Report-level failed-test reruns are not offered for VSO.ai runs because its
dynamic test plan can assign a different seed on the next invocation.
Rerun manifests contain executable local simulator and URG command paths. Run
only manifests generated by simmer in your own report directory; do not run a
downloaded or modified manifest.
...
<report-dir>/regression_report/<project>/<bench>/<timestamp>.rerun.json
