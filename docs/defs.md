...
| <a id="verilog_dv_tb-vcs_three_step"></a>vcs_three_step |  Use VCS three-step analysis/elaboration. Each Bazel library filelist is analyzed by a separate `vlogan -incr_vlogan` command so unchanged libraries such as frozen VIP can skip source parsing. When enabled, `extra_compile_args` must be empty; put analysis options in `vcs_vlogan_args` and elaboration options in `vcs_elab_args`.   | Boolean | optional | `False` |
| <a id="verilog_dv_tb-vcs_vlogan_args"></a>vcs_vlogan_args |  Additional VCS analysis options used only by `vlogan` in `vcs_three_step` mode. Put preprocessor defines and other source-analysis options here. Files referenced with `$(location)` must also be listed in `extra_runfiles`.   | List of strings | optional | [] |
