...
simmer --rerun-report '/path/to/regression_report/project/sys_tb/<timestamp>.rerun.json'
```
...
HTML coverage metrics are VCS-only and appear only when `--vcs-cm` is enabled.
VCS reports without coverage and XRUN reports omit all coverage cards and
columns. Xcelium coverage generation and IMC merge remain available to the
Xcelium workflow, but are not published in the regression HTML dashboard.
When a project table mixes covered VCS benches with XRUN or uncovered benches,
the aggregate table omits coverage columns; each covered VCS bench detail page
still shows its metrics.

For a failed VCS coverage regression, simmer copies the successful-test merged
VDB to a run-specific baseline under `<regression-dir>/report_coverage/`, keeps
the original merged VDB in place, and writes one rerun manifest per affected
bench. The report button copies the corresponding command:

```bash
simmer --rerun-report '/absolute/path/to/<timestamp>.rerun.json'
```

The command reruns each failed seed with its original simulator options and
without generating intermediate reports. Simmer gives the simulation result a
unique suffix while reusing the original VCOMP directory. A successful rerun
copies the complete simulation-produced VDB while the child still owns the VCS
runtime lock. URG then merges
the preserved baseline and successful supplements into a new baseline and a
new timestamped report revision. Failed reruns still contribute no coverage;
if some remain, the revision contains a new manifest for only those failures.
The original timestamped report and baseline remain immutable until normal
30-run report retention expires them. Each single-seed rerun is iteration `i1`;
the supplement artifact name retains the failed run's original iteration.
VSO.ai reports do not emit failed-test rerun manifests because the dynamic VSO
test plan cannot guarantee that a new invocation reuses the original seed.
The manifest records executable local rerun and URG command paths. Treat it as
trusted local build output and run only unmodified manifests generated by
simmer in your own report directory.
...
Use the report-level `simmer --rerun-report <manifest>` command instead of an
individual `rerun.sh` when the successful result must be merged back into a
failed VCS coverage regression. Static HTML cannot start a workstation process;
its rerun button copies the validated CLI command for execution in the existing
EDA environment.
